...
void *cliSvr(void *arg)
{
    ...
    printf("Server:: Requested file : %s\n",FPATH);
     ...
     if ((fileptr = fopen(FPATH, "r+")) == NULL ) {
        ...
        memset(rmsg, '\0', sizeof(rmsg));
        strcpy(rmsg, "/filenotfound.html");
        FPATH = malloc(strlen(ROOT)+strlen(rmsg)+1);
        strcpy(FPATH, ROOT);
        strcat(FPATH, rmsg);
        fileptr1 = fopen(FPATH, "r+");
        fread(&buffer, sizeof(buffer), 1, fileptr1);
                    //fprintf(buffer, "hi there\n");
                    strcpy(buffer2, http_not_found);
...
int main(int argc, char *argv[])
{
     ...
     if (argc < 2) {
         ...
         fprintf(stderr,"Default port: 8950 Selected\n");
         port = 8950;
     }
     else{
        port = atoi(argv[1]);
